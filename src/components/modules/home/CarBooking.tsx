...
import { useState } from "react"
import { useRouter } from "next/navigation"
import { ChevronRight, MapPin, CarFront, Luggage, Sofa, Clock, Calendar, Users, Briefcase, ChevronLeft, ArrowRightLeft } from "lucide-react"
import { Card, CardBody, CardFooter, CardHeader } from "@heroui/card"
import { Button } from "@heroui/button"
import { Spinner } from "@heroui/spinner"
import Image from "next/image"
...
export default function CarBooking() {
  const router = useRouter()
  const [bookingStep, setBookingStep] = useState<"locations" | "packages">("locations") // Start directly at locations step
  const [selectedPickup, setSelectedPickup] = useState<string>("")
  const [selectedDrop, setSelectedDrop] = useState<string>("")
  const [selectedPackage, setSelectedPackage] = useState<string>("")
  ...
  return (
    <Card className="w-full shadow-none bg-inherit">
          ...
          bookingStep !== "locations" && (
            <CardHeader className="flex justify-end">
              <Button
                onPress={handleReset}
                color="primary"
                className="bg-gradient-to-r from-orange-400 to-red-600 text-white rounded-md shadow-md hover:from-orange-500 hover:to-red-700 transition-all"
                startContent={<ChevronLeft className="h-5 w-5 text-white" />}
              >
                Start Over
              </Button>
            </CardHeader>
      ...
      <CardBody>
        {/* Locations Step */}
        {bookingStep === "locations" && (
          <div className="flex flex-col gap-6 py-8 px-4 w-full max-w-3xl mx-auto">
            <h3 className="text-2xl font-semibold text-gray-800 mb-6 text-center">Select Pickup & Drop Locations</h3>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <ArrowRightLeft className="h-12 text-gray-500" />
                  </div>
                  {/* Drop Location */}
                  <div className="w-full">
                    <label htmlFor="drop" className="block text-sm font-medium text-gray-700 mb-2">
                ...
                pkg.carPricing.map((pricing: any) => (
                  ...
                  >
                    <CardBody className="flex flex-col p-0">
                      ...
                      {/* Car Details */}
                      <div className="p-6">
                        <h4 className="text-lg font-semibold text-gray-800 mb-2">{pricing.car.name}</h4>
                        <p className="text-sm text-gray-600 mb-4">{pricing.car.model}</p>
                        ...
                        <div className="flex gap-6 mb-4">
                          ...
                          <div className="flex items-center gap-2 text-gray-600">
                            <div className="flex justify-center items-center gap-1">
                              <Sofa className="text-lg text-primary" />
                              <h1 className="text-gray-600">seats:</h1>
                            </div>
                            <span className="text-sm font-medium text-gray-800">{pricing.car.seats}</span>
                          </div>
                          ...
                          <div className="flex items-center gap-2 text-gray-600">
                            <div className="flex justify-center items-center gap-1">
                              <Luggage className="text-lg text-red-400" />
                              <h1 className="text-gray-600">bags:</h1>
                            </div>
                            <span className="text-sm font-medium text-gray-800">{pricing.car.bags}</span>
                          </div>
                        </div>
      ...
      <CardFooter className="flex justify-between max-w-3xl mx-auto mb-6">
        {bookingStep === "packages" && (
          <Button
            color="primary"
            className="w-full h-14 bg-gradient-to-r from-orange-400 to-red-600 text-white text-lg font-semibold rounded-lg shadow-md hover:from-orange-500 hover:to-red-700 transition-all"
